...
/// Uses the gd: prefix for validator suppression.
/// </summary>
public static class GDValidatorSuppressionParser
{
    // Regex patterns for gd: directives
    // gd:ignore = GD1001, GD2001  or  gd:ignore
    private static readonly Regex IgnorePattern = new Regex(
        @"#\s*gd\s*:\s*ignore\s*(=\s*([\w\-]+(?:\s*,\s*[\w\-]+)*))?",
        RegexOptions.IgnoreCase | RegexOptions.Compiled);

    // gd:disable = GD1001, GD2001  or  gd:disable
    private static readonly Regex DisablePattern = new Regex(
        @"#\s*gd\s*:\s*disable\s*(=\s*([\w\-]+(?:\s*,\s*[\w\-]+)*))?",
        RegexOptions.IgnoreCase | RegexOptions.Compiled);

    // gd:enable = GD1001, GD2001  or  gd:enable
    private static readonly Regex EnablePattern = new Regex(
        @"#\s*gd\s*:\s*enable\s*(=\s*([\w\-]+(?:\s*,\s*[\w\-]+)*))?",
        RegexOptions.IgnoreCase | RegexOptions.Compiled);
    ...
    public static GDValidatorSuppressionContext Parse(GDNode? node)
    {
        var directives = new List<GDValidatorSuppressionDirective>();

        if (node == null)
            return new GDValidatorSuppressionContext(directives);
    ...
    internal static GDValidatorSuppressionDirective? ParseComment(GDComment comment)
    {
        if (comment == null)
            return null;
        ...
        if (ignoreMatch.Success)
        {
            ...
            return new GDValidatorSuppressionDirective(GDValidatorSuppressionType.Ignore, ruleIds, line, isInline);
        }
        ...
        if (disableMatch.Success)
        {
            ...
            return new GDValidatorSuppressionDirective(GDValidatorSuppressionType.Disable, ruleIds, line, isInline);
        }
        ...
        if (enableMatch.Success)
        {
            ...
            return new GDValidatorSuppressionDirective(GDValidatorSuppressionType.Enable, ruleIds, line, isInline);
        }
    ...
    private static bool IsInlineComment(GDComment comment)
    {
        ...
        while (prev != null)
        {
            ...
            if (prev is not GDSpace && prev is not GDIntendation)
                return true;
