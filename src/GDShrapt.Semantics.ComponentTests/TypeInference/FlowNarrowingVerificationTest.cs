...
public class FlowNarrowingVerificationTest
{
    ...
    private static string OutputPath => Path.Combine(GetSolutionRoot(), "FLOW_NARROWING_OUTPUT.txt");
    private static string VerifiedPath => Path.Combine(GetSolutionRoot(), "FLOW_NARROWING_VERIFIED.txt");
    ...
    private void WriteUnverifiedReport(GDScriptProject project, string projectPath, Dictionary<string, FlowNarrowingVerificationParser.VerifiedNarrowing> verifiedLookup)
    {
        ...
        var unverifiedPath = Path.Combine(GetSolutionRoot(), "FLOW_NARROWING_UNVERIFIED.txt");
