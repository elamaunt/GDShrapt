...
internal class GDCallableStringFlowRegistry
{
    private readonly object _lock = new();
    private readonly Dictionary<string, List<GDCallableStringFlowEntry>> _byMethodName = new(StringComparer.Ordinal);

    public void Register(GDCallableStringFlowEntry entry)
    {
        lock (_lock)
        {
            if (!_byMethodName.TryGetValue(entry.ResolvedMethodName, out var list))
            {
                list = new List<GDCallableStringFlowEntry>();
