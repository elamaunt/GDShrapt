...
public class GDDepsCommand : GDProjectCommandBase
{
    ...
    private void WriteSingleFileOutput(GDFileDependencyInfo info, string projectRoot)
    {
        ...
        _formatter.WriteMessage(_output, $"Dependencies for: {relPath}");
        _formatter.WriteMessage(_output, "");

        if (!string.IsNullOrEmpty(info.ExtendsClass))
        {
            _formatter.WriteMessage(_output, $"  Extends: {info.ExtendsClass}");
        }
        else if (!string.IsNullOrEmpty(info.ExtendsScript))
        {
            _formatter.WriteMessage(_output, $"  Extends: {info.ExtendsScript}");
        }
        ...
        if (info.Preloads.Count > 0)
        {
            ...
            {
                _formatter.WriteMessage(_output, $"    - {preload}");
            }
        }
        ...
        if (info.Loads.Count > 0)
        {
            ...
            {
                _formatter.WriteMessage(_output, $"    - {load}");
            }
        }

        if (info.Dependencies.Count > 0)
        {
            _formatter.WriteMessage(_output, $"  Transitive Dependencies: {info.Dependencies.Count}");
            foreach (var dep in info.Dependencies.Take(10))
            {
                var depRel = GetRelativePath(dep, projectRoot);
                _formatter.WriteMessage(_output, $"    - {depRel}");
            }
            if (info.Dependencies.Count > 10)
                _formatter.WriteMessage(_output, $"    ... and {info.Dependencies.Count - 10} more");
        }

        if (info.Dependents.Count > 0)
        {
            _formatter.WriteMessage(_output, $"  Dependents (files that depend on this): {info.Dependents.Count}");
            ...
            if (info.Dependents.Count > 10)
                _formatter.WriteMessage(_output, $"    ... and {info.Dependents.Count - 10} more");
        }

        if (info.IsInCycle)
        {
            _formatter.WriteMessage(_output, "");
            _formatter.WriteMessage(_output, "  WARNING: This file is part of a circular dependency!");
            if (info.CycleMembers != null && info.CycleMembers.Count > 0)
            {
                ...
                foreach (var member in info.CycleMembers)
                {
                    ...
                    _formatter.WriteMessage(_output, $"    - {memberRel}");
                }
            }
        }
    }
    ...
    private void WriteProjectOutput(GDProjectDependencyReport report, string projectRoot, GDProjectSemanticModel projectModel)
    {
        ...
        _formatter.WriteMessage(_output, $"  Circular Dependencies: {report.CycleCount}");
        _formatter.WriteMessage(_output, $"  Files in Cycles: {report.FilesInCycles}");
        _formatter.WriteMessage(_output, "");

        // Show cycles
        if (report.HasCycles)
        {
            _formatter.WriteMessage(_output, "Circular Dependencies:");
            int cycleNum = 1;
        ...
        if (_options.ShowCoupled)
        {
            ...
            {
                _formatter.WriteMessage(_output, "Most Coupled Files (most dependencies):");
                foreach (var file in mostCoupled)
                {
                    var relPath = GetRelativePath(file.FilePath, projectRoot);
                    _formatter.WriteMessage(_output, $"  {relPath}: {file.DirectDependencyCount} direct deps");
                }
                _formatter.WriteMessage(_output, "");
            }
        }
        ...
        if (_options.ShowDependent)
        {
            ...
            {
                _formatter.WriteMessage(_output, "Most Depended-On Files (most dependents):");
