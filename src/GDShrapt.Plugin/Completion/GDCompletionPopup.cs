...
/// </summary>
internal partial class GDCompletionPopup : PopupPanel
{
    private ItemList _itemList;
    private List<GDCompletionItem> _allItems = new();
    private List<GDCompletionItem> _filteredItems = new();
    ...
    /// </summary>
    public event Action<GDCompletionItem>? ItemSelected;
        ...
        string initialFilter = "")
    {
        ...
        {
            Logger.Debug("GDCompletionPopup: No matching completions");
            return;
        }
    ...
    /// </summary>
    public GDCompletionItem? GetSelectedItem()
    {
    ...
    private string FormatItemText(GDCompletionItem item)
    {
        var text = item.Label;

        // Add type hint for variables/properties
        if (!string.IsNullOrEmpty(item.TypeName) && item.Kind != GDCompletionItemKind.Keyword)
        {
            text += $"  : {item.TypeName}";
        }

        // Add detail for methods
        if (!string.IsNullOrEmpty(item.Detail) && item.Kind == GDCompletionItemKind.Method)
        {
            text += $"  {item.Detail}";
        }
    ...
    private void InsertCompletion(GDCompletionItem item)
    {
        if (_textEdit == null)
            return;
        ...
        {
            Logger.Debug("GDCompletionPopup: Cursor moved to different line, skipping insertion");
            return;
        }
        ...
        try
        {
            ...
            if (item.Kind == GDCompletionItemKind.Method && insertText.EndsWith("()"))
            {
        ...
        Logger.Debug($"GDCompletionPopup: Inserted '{insertText}' at line {cursorLine}, column {replaceStart}");
    }
    ...
    private Texture2D? GetKindIcon(GDCompletionItemKind kind)
    {
        try
        {
            ...
            {
                GDCompletionItemKind.Method => "MemberMethod",
                GDCompletionItemKind.Property => "MemberProperty",
                GDCompletionItemKind.Variable => "MemberProperty",
                GDCompletionItemKind.Event => "MemberSignal",
                GDCompletionItemKind.Constant => "MemberConstant",
                GDCompletionItemKind.Class => "Object",
                GDCompletionItemKind.Enum => "Enum",
                GDCompletionItemKind.EnumMember => "MemberConstant",
                GDCompletionItemKind.Keyword => "Shortcut",
                GDCompletionItemKind.Snippet => "Script",
                _ => null
            };
        ...
        {
            Logger.Debug($"GDCompletionPopup: Could not get icon: {ex.Message}");
        }
