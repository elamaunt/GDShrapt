...
/// </summary>
internal class GDTodoTagsScanner : IDisposable
{
    private readonly GDScriptProject _scriptProject;
    private readonly GDConfigManager _configManager;
    private readonly ConcurrentDictionary<string, List<GDTodoItem>> _cache = new();
    ...
    /// </summary>
    public event Action<GDTodoTagsScanResult>? OnScanCompleted;

    /// <summary>
    /// Fired when a single file is rescanned.
    /// </summary>
    public event Action<string, List<GDTodoItem>>? OnFileScanned;

    public GDTodoTagsScanner(GDScriptProject ScriptProject, GDConfigManager configManager)
    {
    ...
    /// </summary>
    public async Task<GDTodoTagsScanResult> ScanProjectAsync(CancellationToken cancellationToken = default)
    {
        var result = new GDTodoTagsScanResult
        {
    ...
    /// </summary>
    public Task<List<GDTodoItem>> ScanScriptAsync(GDScriptFile script, CancellationToken cancellationToken = default)
    {
        var items = new List<GDTodoItem>();
    ...
    /// </summary>
    public GDTodoTagsScanResult GetCachedResults()
    {
        var result = new GDTodoTagsScanResult
        {
            ScanTime = DateTime.UtcNow
