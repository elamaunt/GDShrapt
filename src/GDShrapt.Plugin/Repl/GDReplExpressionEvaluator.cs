...
/// </summary>
internal class GDReplExpressionEvaluator
{
    /// <summary>
    /// Evaluates an expression in the context of a target GodotObject.
    /// </summary>
    public GDReplResult Evaluate(GDExpression expression, GodotObject target)
    {
        if (expression == null)
            return GDReplResult.Error("Expression is null");

        if (target == null)
            return GDReplResult.Error("Target node is null");

        try
        {
            var result = EvaluateExpression(expression, target);
            return GDReplResult.WithValue(result);
        }
        catch (Exception ex)
        {
            return GDReplResult.Error(ex.Message);
        }
    }
