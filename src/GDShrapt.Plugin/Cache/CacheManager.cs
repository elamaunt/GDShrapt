...
internal class CacheManager : IDisposable
{
    ...
    /// </summary>
    public CacheManager(string projectPath)
    {
    ...
    /// </summary>
    public bool TryGetLintCache(ScriptReference script, string contentHash, out IReadOnlyList<Diagnostic> diagnostics)
    {
        ...
        try
        {
            ...
            var entry = JsonSerializer.Deserialize<LintCacheEntry>(json, JsonOptions);
    ...
    /// </summary>
    public void StoreLintCache(ScriptReference script, string contentHash, IReadOnlyList<Diagnostic> diagnostics)
    {
        try
        {
            var key = GetCacheKey(script);
            ...
            // Create cache entry
            var entry = new LintCacheEntry
            {
                ContentHash = contentHash,
                AnalyzedAt = DateTime.UtcNow,
                Diagnostics = diagnostics.Select(SerializedDiagnostic.FromDiagnostic).ToList()
            };
            ...
            // Update index
            _index.Files[key] = new CacheFileInfo
            {
                ContentHash = contentHash,
    ...
    /// </summary>
    public void Invalidate(ScriptReference script)
    {
        try
        {
            var key = GetCacheKey(script);
    ...
    private void LoadIndex()
    {
        ...
        _index = new CacheIndex { Version = CurrentCacheVersion };
    }
    ...
    private static string GetCacheKey(ScriptReference script)
    {
