...
func test_string_format():
	var result = "Value: %d" % 42  # Valid
	var result2 = "Values: %s" % [1, 2, 3]  # Valid
	var result3 = "Count: %d, Name: %s" % [42, "test"]  # Valid
	var result4 = "Percent: %d%%" % 50  # Valid
...
func test_stringname():
	var sn: StringName = "test"  # Valid implicit conversion
	var sn2: StringName = &"test"  # Explicit StringName literal

	var s: String = sn  # Valid implicit conversion back

	# Using String where StringName expected
	var name: StringName = get_string_value()
